...
const MANAGER_PAGE_TIMING_ENABLED =
  process.env.NODE_ENV !== "production" ||
  process.env.DASHBOARD_DEBUG_TIMING === "true";

function createManagerPageTimer(scope: string): (label: string) => void {
  const startedAt = performance.now();

  return (label: string): void => {
    if (!MANAGER_PAGE_TIMING_ENABLED) {
      return;
    }
...
export default async function ManagerDashboardPage(): Promise<JSX.Element> {
  const mark = createManagerPageTimer("manager:dashboard");
