...
export function AsyncEntityCombobox<TItem>({
  ...
  const activeDescendant =
    activeIndex >= 0 ? `${overlayId}-${activeIndex}` : undefined;
  ...
  return (
    <div ref={rootRef} className={cn("relative min-w-0", className)}>
      ...
      <div className="flex min-w-0 items-center gap-2 border border-border bg-surface px-3 py-2 shadow-sm transition focus-within:border-primary/50 focus-within:ring-2 focus-within:ring-primary/10">
        <input
          ref={inputRef}
          id={id}
          type="search"
      ...
      {showOverlay ? (
        <div
          id={overlayId}
          role="listbox"
          className="absolute left-0 right-0 top-[calc(100%+0.4rem)] z-50 max-h-[24rem] overflow-y-auto border border-border bg-surface shadow-xl"
        >
          {items.length > 0 ? (
              ...
              {items.map((item, index) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "block w-full border border-transparent px-3 py-2.5 text-left transition",
                      active
